...
void CImageToolDoc::SelectFolder( LPCWSTR pstr )
{
	...
	pView->m_previewDlg.SendMessage( WM_SIZE );
...
void CImageToolDoc::SelectItem( int nIndex )
{
	...
	if( nIndex < m_vFileName.size() )
	{
        ...
        CImageToolView* pView = (CImageToolView*)(pFrame)->GetActiveView();
		//pView->m_pPreviewDlg->SendMessage( WM_SIZE );

        LoadBitmapFromFile((LPCWSTR)filePath)
            .then([pFrame, pView](ComPtr<IWICBitmapSource> pSource)
        {
            pView->m_previewDlg.m_wndCanvas.SetWicBitmap(pSource);
            pFrame->m_wndThumbnailView.SetWicBitmap(pSource);
            pFrame->m_wndThumbnailView.FitScaleToBitmap();
            pFrame->m_wndMagnifyView.SetWicBitmap(pSource);
        })
            .then([pFrame, pView]
        {
            pView->m_previewDlg.m_wndCanvas.Invalidate();
            pFrame->m_wndThumbnailView.Invalidate();
            pFrame->m_wndMagnifyView.Invalidate();
        });
	}
}
...
void CImageToolDoc::OnEditUndo() 
{
    ...
    Gdiplus::Bitmap *tmpImg = m_pSelectedImage;
...
void CImageToolDoc::OnUpdateEditUndo(CCmdUI* pCmdUI) 
{
	...
	CString s; s.Format( L"Undo (%d)", m_UndoLevel );
